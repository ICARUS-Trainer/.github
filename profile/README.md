...
```mermaid
flowchart TD
    A[Trainer Launch] --> B[ICARUS Process Found]
    B --> C[Module Load]
    C --> D{Player Toggles}
    D -->|Survival| E[Stamina / Oxygen]
    D -->|Environment| F[Storm / Weather]
    D -->|Crafting| G[Build & Recipes]
    E --> H[Live Memory Override]
    F --> H
    G --> H
    H --> I[Immediate Gameplay Response]
```
